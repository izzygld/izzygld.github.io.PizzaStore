...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    var test2 = document.querySelector("#pizzaSize")
    switch(size) {
      case "1":
        return;
        test2.innerHTML = "Small";
        return;
      case "2":
        test2.innerHTML = "Medium";
        return;
      case "3":
        test2.innerHTML = "Large";
        return;
      default:
  ...
  // We calculate the window width and the element width only once saving us a lot of time.
  function changePizzaSizes(size) {
    var test = document.getElementsByClassName("randomPizzaContainer");
    var windowwidth = document.getElementById("randomPizzas").offsetWidth;
    var elemWidth = test[0].offsetWidth;
    for (var i = 0; i < test.length; i++) {
      var dx = determineDx(elemWidth, size, windowwidth);
      var newwidth = (elemWidth + dx) + 'px';
      test[i].style.width = newwidth;
    }
  }
...
// Moving the pizza Div allows me to only call the DOM once.
...
//took variables outside of the for loop
//replaced querySelectorAll with getElementsByClassName
//Stored phase values in array
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    //using Transform instead of style.left
...
window.addEventListener('scroll', pizzasDance);


//Change the amount of pizzas rendered in the background from 200 to 30 (save time)
